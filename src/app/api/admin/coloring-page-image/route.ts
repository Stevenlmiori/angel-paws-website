...
async function saveLocalUpload(
  ...
  const dir = path.join(process.cwd(), "public", "coloring-pages", "uploads");
  await mkdir(dir, { recursive: true });
  await writeFile(path.join(dir, filename), buffer);
  return `/coloring-pages/uploads/${filename}`;
}
...
export async function POST(request: Request) {
  ...
  if (!client) {
    if (process.env.NODE_ENV !== "production") {
      ...
      const upload = await readVerifiedImageUpload(file, "coloring-upload.jpg");
      if (!upload.ok) {
      ...
      const src = await saveLocalUpload(
        upload.value.buffer,
