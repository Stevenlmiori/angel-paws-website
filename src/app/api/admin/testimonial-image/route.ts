...
async function saveLocalUpload(buffer: Buffer, filenameBase: string): Promise<string> {
  ...
  const dir = path.join(process.cwd(), "public", "img", "testimonials", "uploads");
  await mkdir(dir, { recursive: true });
  await writeFile(path.join(dir, filename), buffer);
  return `/img/testimonials/uploads/${filename}`;
}
...
export async function POST(request: Request) {
  ...
  const upload = await readVerifiedImageUpload(file, "testimonial-upload.jpg");
  if (!upload.ok) {
