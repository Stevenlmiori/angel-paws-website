...
**Where We Serve (home tiles tucked under “They show up”)**

- No section heading/copy on Home (to avoid repetition with the section above).
...
- Hero image: `sam-comforting-kid-1.jpg` (`SITE_IMAGES.ministrySamComfortingKid1` in `src/components/pet-therapy/media.ts`).
- Floating card on the hero image (large screens): “Connection without an agenda—just presence, patience, and a gentle spirit.”
